...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed z-[999] w-full px-14 py-6 flex justify-between items-center transition-transform duration-300 ${
      ...
      <div className="links flex gap-6 text-[#212121]">
        {["Services", "Our work", "About us", "Insights", "Contact us"].map((item, index) => (
          <a href="#" key={index} className={`text-[1.1vw] font-regular font-display ${index === 4 && `ml-64`}`}>{item}</a>
        ))}
      </div>
    </nav>
  );
